...
public class DefaultDiskStorageSupplier implements DiskStorageSupplier {
	...
	static class State {
		...
		@VisibleForTesting
		State(File rootDirectory, DiskStorage storage) {
			this.storage = storage;
			this.rootDirectory = rootDirectory;
		}
	}

	public DefaultDiskStorageSupplier(int version, Supplier<File> baseDirectoryPathSupplier, String baseDirectoryName,
	...
	/**
	 * Gets a concrete disk-storage instance. If nothing has changed since the
	 * last call, then the last state is returned
	 * 
	...
	private boolean shouldCreateNewStorage() {
		...
		return (currentState.storage == null || currentState.rootDirectory == null || !currentState.rootDirectory
				.exists());
	}
	...
	void deleteOldStorageIfNecessary() {
		if (mCurrentState.storage != null && mCurrentState.rootDirectory != null) {
			// LATER: Actually delegate this call to the storage. We shouldn't
			// be
	...
	void createRootDirectoryIfNecessary(File rootDirectory) throws IOException {
		try {
			...
			mCacheErrorLogger.logError(CacheErrorLogger.CacheErrorCategory.WRITE_CREATE_DIR, TAG,
					"createRootDirectoryIfNecessary", cde);
			throw cde;
		}
