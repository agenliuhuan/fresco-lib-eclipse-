...
public class DiskStorageCache implements FileCache, DiskTrimmable {
	...
	/**
	 * Stats about the cache - currently size of the cache (in bytes) and number
	...
	public DiskStorageCache(DiskStorageSupplier diskStorageSupplier, Params params,
			CacheEventListener cacheEventListener, CacheErrorLogger cacheErrorLogger,
	...
	/**
	 * Probes whether the object corresponding to the mKey is in the cache. Note
	 * that the act of probing touches the item (if present in cache), thus
	 * changing its LRU timestamp.
	 * <p>
	 * This will be faster than retrieving the object, but it still has file
	 * system accesses and should NOT be called on the UI thread.
	 * 
	 * @param key
	 *            the mKey to check
	 ...
	 */
	private BinaryResource createTemporaryResource(final String resourceId, final CacheKey key) throws IOException {
	...
	/**
	 * Commits the provided temp file to the cache, renaming it to match the
	 * cache's hashing convention.
	 */
	private BinaryResource commitResource(final String resourceId, final CacheKey key, final BinaryResource temporary)
			throws IOException {
		synchronized (mLock) {
	...
	public BinaryResource insert(CacheKey key, WriterCallback callback) throws IOException {
		// Write to a temp file, then move it into place. This allows more
		// parallelism
		// when writing files.
	...
	public void remove(CacheKey key) {
		synchronized (mLock) {
			try {
				...
				mCacheErrorLogger.logError(CacheErrorLogger.CacheErrorCategory.DELETE_FILE, TAG,
	...
	public long clearOldEntries(long cacheExpirationMs) {
		...
		synchronized (mLock) {
			try {
				...
				mCacheErrorLogger.logError(CacheErrorLogger.CacheErrorCategory.EVICTION, TAG,
						"clearOldEntries: " + ioe.getMessage(), ioe);
			}
		}
	...
	/**
	 * Test if the cache size has exceeded its limits, and if so, evict some
	 * files. It also calls maybeUpdateFileCacheSize
	 * 
	...
	private void evictAboveSize(long desiredSize, CacheEventListener.EvictionReason reason) throws IOException {
		...
		try {
			...
			mCacheErrorLogger.logError(CacheErrorLogger.CacheErrorCategory.EVICTION, TAG,
					"evictAboveSize: " + ioe.getMessage(), ioe);
			throw ioe;
		}
	...
	/**
	 * If any file timestamp is in the future (beyond now +
	 * FUTURE_TIMESTAMP_THRESHOLD_MS), we will set its effective timestamp to 0
	 * (the beginning of unix time), thus sending it to the head of the queue
	 * for eviction (entries with the lowest timestamps are evicted first). This
	 * is a safety check in case we get files that are written with a future
	 * timestamp. We are adding a small delta (this constant) to account for
	...
	private Collection<DiskStorage.Entry> getSortedEntries(Collection<DiskStorage.Entry> allEntries) {
		final ArrayList<DiskStorage.Entry> entriesList = new ArrayList<DiskStorage.Entry>(allEntries);
	...
	/**
	 * Compares file timestamps, but files with timestamps more than some future
	 * threshold are considered to have a timestamp of zero so they are sent to
	 * the head of the queue for eviction.
	 */
	...
	/**
	 * Helper method that sets the cache size limit to be either a high, or a
	 * low limit. If there is not enough free space to satisfy the high limit,
	 * it is set to the low limit.
	 */
	private void updateFileCacheSizeLimit() {
		// Test if mCacheSizeLimit can be set to the high limit
		boolean isAvailableSpaceLowerThanHighLimit = mStatFsHelper.testLowDiskSpace(StatFsHelper.StorageType.INTERNAL,
	...
	private void trimBy(final double trimRatio) {
		synchronized (mLock) {
			try {
				...
				mCacheErrorLogger.logError(CacheErrorLogger.CacheErrorCategory.EVICTION, TAG,
						"trimBy: " + ioe.getMessage(), ioe);
			}
		}
	}

	/**
	 * If file cache size is not calculated or if it was calculated a long time
	 * ago (FILECACHE_SIZE_UPDATE_PERIOD_MS) recalculated from file listing.
	 * 
	...
	private boolean maybeUpdateFileCacheSize() {
		...
		if ((!mCacheStats.isInitialized()) || mCacheSizeLastUpdateTime == UNINITIALIZED
	...
	private void calcFileCacheSize() {
		...
		try {
			...
			for (DiskStorage.Entry entry : entries) {
				...
				// Check if any files have a future timestamp, beyond our
				// threshold
			...
			if (foundFutureTimestamp) {
				mCacheErrorLogger.logError(CacheErrorLogger.CacheErrorCategory.READ_INVALID_ENTRY, TAG,
						"Future timestamp found in " + numFutureFiles + " files , with a total size of "
								+ sizeFutureFiles + " bytes, and a maximum time delta of " + maxTimeDelta + "ms", null);
			}
			mCacheStats.set(size, count);
		} catch (IOException ioe) {
			mCacheErrorLogger.logError(CacheErrorLogger.CacheErrorCategory.GENERIC_IO, TAG,
					"calcFileCacheSize: " + ioe.getMessage(), ioe);
		}
	}
